...
void MapMaker::intitialise()
{
    ...
    //check that there isn't an existing fMap and clear it if there is
    //prevents maps stacking
    ...
    for(int k=0; k<7; k++)
    {
            ...
            for(int j=0; j<fYSize; j++)
        {
            ...
            if(inBounds(std::vector<int>{i-1, j-1})==false) count+=0.7; //comment out these lines for an easy alternate map style
