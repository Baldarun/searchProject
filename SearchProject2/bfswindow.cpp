...
void BFSWindow::paintEvent(QPaintEvent *)
{
    ...
    for(int i=0; i<fBFS->getPath().size(); i++)
    {
        //draws this object in the top right corner so the explored status is still visible
        painter.fillRect(fBFS->getPath()[i][1]*30+15, fBFS->getPath()[i][0]*30, 15, 15, Qt::green);
    }

    //a loop that runs over every value in the map to draw it.
